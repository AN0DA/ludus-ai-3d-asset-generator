...
class UIHandlers:
    ...
    def generate_asset_sync(
        ...
        try:
            logger.info("Starting generate_asset_sync wrapper")
            ...
            # Create the async task using the main app's event loop
            # We need to schedule this in the app's running event loop
            ...
            def run_in_thread():
                nonlocal result, exception, task_id
                try:
                    logger.info("Creating new event loop in thread")
                    # Create a new event loop for this thread
                    new_loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(new_loop)
                    logger.info("Event loop created, running task creation")
                    
                    async def create_background_task():
                        """Create the background task and return immediately."""
                        # Call the generate_asset_pipeline method to start the task
                        tid, sid = await self.app.generate_asset_pipeline(
                            description=description.strip(),
                            asset_type=asset_type_enum,
                            style_preference=style_enum,
                            quality_level=quality_enum,
                    ...
                    try:
                        logger.info("Running task creation")
                        task_id, session_id = new_loop.run_until_complete(create_background_task())
                        logger.info("Task creation completed", task_id=task_id)
                        
                        # Just return success - the task will continue in background
                        result = (
                            '''
                            <div class="status-success">
                                🚀 <strong>Generation Started!</strong>
                    ...
                    logger.error("Exception in thread", error=str(e), exc_info=True)
                    exception = e
            ...
            if thread.is_alive():
                ...
                error_msg = '''
                <div class="status-error">
                    ❌ <strong>Timeout Error:</strong> Generation request timed out.
                    <br><small>Please try again with a simpler request.</small>
                </div>
                '''
                return "", "", error_msg, False
            
            if exception:
                logger.error("Exception occurred in generation", error=str(exception))
                raise exception
                
            # Store the task ID for progress monitoring
            if task_id:
                self.current_task_id = task_id
            
            logger.info("Generation sync wrapper completed successfully")
            return result if result else ("", "", "Unknown error occurred", False)
        ...
        try:
            ...
            def run_in_thread():
                ...
                try:
                    ...
                    asyncio.set_event_loop(new_loop)
                    try:
                        result = new_loop.run_until_complete(self.cancel_generation_async())
                    finally:
                        new_loop.close()
                except Exception as e:
                    exception = e
            ...
            thread.daemon = True
            thread.start()
            thread.join(timeout=10)  # 10 second timeout for cancel
            
            if thread.is_alive():
                logger.error("Cancel request timed out")
                return '''
                <div class="status-error">
                    ❌ <strong>Cancel Timeout:</strong> Unable to cancel generation.
                </div>
                ''', "", False
    ...
    def _validate_inputs(self, description: str, asset_type: str, style: str, quality: str) -> Optional[str]:
